...
def get_item_information(item_code):
  ...
  if item_code in data.menu_items:
        item_details = data.menu_items[item_code]
        item_name = item_details[0]
        item_price = item_details[1]
        stock_number = item_details[2] if len(item_details) > 2 else None
        return {
            "code": item_code,
            "name": item_name,
            "price": item_price,
      ...
      raise ValueError("Item code not found in menu.")
  print(item_code)
...
def get_item_number():
  while True:
    ...
    print('Dessert', [d.replace('\u200b','') for d in data.menu_items if d[0] == 'D'])
